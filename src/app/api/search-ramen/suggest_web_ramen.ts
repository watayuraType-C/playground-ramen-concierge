...
export async function suggest_web_ramen(
  ...
  const modelNames = ["gemini-1.5-flash", "gemini-2.5-flash", "gemini-flash-latest"];
  ...
  for (const modelName of modelNames) {
    try {
      ...
      // JSONとしてパースして返す（Markdownが混入した場合の対策）
      ...
      return JSON.parse(jsonString) as FinalRamenData;

    } catch (error: any) {
      console.warn(`Web Suggestion failed with ${modelName}:`, error.message);
    }
  }

  console.error("Web Suggestion Error: All models failed.");
  return null;
}